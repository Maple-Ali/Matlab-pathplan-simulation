%% exp_aco_pr136 — TSP算法在pr136数据集上的性能测试
%  数据集: pr136 (136城市, EUC_2D坐标, 预计算距离矩阵)
%  起点/终点: 城市1 (同一点)
%  运行次数: 20次独立运行
...
% ===== Experiment Config =====
nRuns = 10;
coordFile = fullfile(fileparts(mfilename('fullpath')), 'pr136_coords.mat');
distFile  = fullfile(fileparts(mfilename('fullpath')), 'pr136_distance_matrix.txt');
resultsDir = fullfile(fileparts(mfilename('fullpath')), 'results');

%% ===== Load pr136 dataset =====
tmp = load(coordFile);  % → c136 (136×2)
cityCoords = tmp.c136;
fullDist136 = parsePr136Dist(distFile);
nCities = 136;

fprintf('======== TSP Experiment: pr136 ========\n');
fprintf('Algorithm: %s\n', func2str(tspSolver));
fprintf('Cities: %d | Runs: %d\n', nCities, nRuns);
fprintf('Start/Goal: City 1 (%.0f, %.0f)\n', cityCoords(1,1), cityCoords(1,2));
fprintf('Known optimal: 96772\n\n');

%% ===== Build 137x137 cost matrix (start=city1, goal=city1) =====
nPts = nCities + 1;  % 137: [1=start, 2..136=cities2..136, 137=goal]
costMatrix = zeros(nPts);
costMatrix(1, 2:nCities)         = fullDist136(1, 2:nCities);
costMatrix(2:nCities, 1)         = fullDist136(2:nCities, 1);
costMatrix(2:nCities, nPts)      = fullDist136(2:nCities, 1);
costMatrix(nPts, 2:nCities)      = fullDist136(1, 2:nCities);
costMatrix(2:nCities, 2:nCities) = fullDist136(2:nCities, 2:nCities);
...
fprintf('Median: %.2f\n', stats.medianCost);
fprintf('Gap to optimal (96772): %.1f%%\n', (stats.bestCost - 96772) / 96772 * 100);
fprintf('Total wall time: %.1fs\n', totalWallTime);

%% ===== Export Data =====
if ~exist(resultsDir, 'dir'), mkdir(resultsDir); end
resultsFile = fullfile(resultsDir, 'exp_aco_pr136_results.mat');
save(resultsFile, 'allOrders', 'allCosts', 'allHistories', 'bestRunIdx', 'stats', ...
    'fullDist136', 'cityCoords', 'costMatrix');
fprintf('\nResults saved to: %s\n', resultsFile);
...
for i = 1:labelStep:nCities
    text(cityCoords(i,1) + 50, cityCoords(i,2) + 50, num2str(i), ...
...
text(cityCoords(1,1) + 120, cityCoords(1,2) - 150, 'Start', ...
    'HorizontalAlignment', 'center', 'FontSize', 10, 'FontWeight', 'bold', 'Color', [0.2, 0.6, 0.2]);

xlabel('X'); ylabel('Y');
title(sprintf('Best TSP Tour — pr136 (Cost: %.2f, Run #%d)', stats.bestCost, bestRunIdx), 'FontSize', 13);
axis equal tight;
...
yline(96772, '--', 'Color', [0.2, 0.7, 0.2], 'LineWidth', 1.0);
xlabel('Iteration'); ylabel('Best Cost');
title('Convergence — Iteration (Median + 95% CI)');
legend('95% CI', 'Median', 'Optimal (96772)', 'Location', 'northeast'); grid on;
hold off;
...
function distMatrix = parsePr136Dist(distFile)
%PARSEPR136DIST Parse pr136 distance matrix
%   distMatrix - 136x136 symmetric distance matrix
