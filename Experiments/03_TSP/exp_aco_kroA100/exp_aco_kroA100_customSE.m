%% exp_aco_kroA100_customSE — 可设定起点/终点城市编号的kroA100 TSP实验
%  数据集: kroA100 (100城市, EUC_2D坐标, 预计算距离矩阵)
%  与 exp_aco_kroA100.m 的区别：可自定义起点和终点城市编号
...
fprintf('======== TSP Experiment: kroA100 (Custom Start/End) ========\n');
fprintf('Algorithm: %s\n', func2str(tspSolver));
...
resultsFile = fullfile(resultsDir, sprintf('exp_kroA100_%s_to_%s.mat', startLabel, goalLabel));
...
xlabel('X'); ylabel('Y');
titleStr = sprintf('Best TSP Tour — kroA100 (C%d→C%d, Cost: %.2f, Run #%d)', ...
