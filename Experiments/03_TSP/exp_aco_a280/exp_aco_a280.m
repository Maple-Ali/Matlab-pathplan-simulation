%% exp_aco_a280 — TSP算法在a280数据集上的性能测试
%  数据集: a280 (280城市, EUC_2D坐标, 预计算距离矩阵)
%  起点/终点: 城市1 (同一点)
%  运行次数: 30次独立运行
...
%% ===== Parse a280 dataset =====
[fullDist280, cityCoords] = parseA280(distFile, tspFile);
nCities = 280;

fprintf('======== TSP Experiment: a280 ========\n');
fprintf('Algorithm: %s\n', func2str(tspSolver));
fprintf('Cities: %d | Runs: %d\n', nCities, nRuns);
fprintf('Start/Goal: City 1 (%.0f, %.0f)\n', cityCoords(1,1), cityCoords(1,2));
fprintf('Known optimal: 2579\n\n');

%% ===== Build 281x281 cost matrix (start=city1, goal=city1) =====
nPts = nCities + 1;  % 281: [1=start, 2..280=cities2..280, 281=goal]
costMatrix = zeros(nPts);
costMatrix(1, 2:nCities)         = fullDist280(1, 2:nCities);
costMatrix(2:nCities, 1)         = fullDist280(2:nCities, 1);
costMatrix(2:nCities, nPts)      = fullDist280(2:nCities, 1);
costMatrix(nPts, 2:nCities)      = fullDist280(1, 2:nCities);
costMatrix(2:nCities, 2:nCities) = fullDist280(2:nCities, 2:nCities);
...
fprintf('Median: %.2f\n', stats.medianCost);
fprintf('Gap to optimal (2579): %.1f%%\n', (stats.bestCost - 2579) / 2579 * 100);
fprintf('Total wall time: %.1fs\n', totalWallTime);

%% ===== Export Data =====
resultsFile = fullfile(resultsDir, 'exp_aco_a280_results.mat');
save(resultsFile, 'allOrders', 'allCosts', 'allHistories', 'bestRunIdx', 'stats', ...
    'fullDist280', 'cityCoords', 'costMatrix');
fprintf('\nResults saved to: %s\n', resultsFile);
...
text(cityCoords(1,1) + 8, cityCoords(1,2) - 12, 'Start', ...
    'HorizontalAlignment', 'center', 'FontSize', 10, 'FontWeight', 'bold', 'Color', [0.2, 0.6, 0.2]);

xlabel('X'); ylabel('Y');
title(sprintf('Best TSP Tour — a280 (Cost: %.2f, Run #%d)', stats.bestCost, bestRunIdx), 'FontSize', 13);
axis equal tight;
...
% Add known optimal reference line
yline(2579, '--', 'Color', [0.2, 0.7, 0.2], 'LineWidth', 1.0);
xlabel('Iteration'); ylabel('Best Cost');
title('Convergence — Iteration (Median + 95% CI)');
legend('95% CI', 'Median', 'Optimal (2579)', 'Location', 'northeast'); grid on;
hold off;
...
function [distMatrix, coords] = parseA280(distFile, tspFile)
%PARSEA280 Parse a280 distance matrix and coordinates
%   distMatrix - 280x280 symmetric distance matrix
%   coords     - 280x2 [x, y] coordinates
    ...
    % Data rows: row i → city i, entries for cities 280 down to i+1
    for i = 1:(nCities - 1)
        line = strtrim(fgetl(fid));
        % Remove "row_num: " prefix
        ...
        nVals = length(vals);
        for p = 1:nVals
            j = nCities - p + 1;  % city index (280, 279, ..., i+1)
            distMatrix(i, j) = vals(p);
            distMatrix(j, i) = vals(p);
        end
    end

    % --- Parse coordinates from a280.tsp ---
