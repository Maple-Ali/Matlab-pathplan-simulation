...
%
%   输入以下内容运行：
%   resultTable = analyzeConvergenceTime('exp_AandACO_Map1_results.mat'); disp(resultTable);
%
%   输入参数：
...
%   - 标准差使用样本标准差（除以 N-1），若需总体标准差可自行修改 std 调用。
    ...
    if any(validIdx)
        ...
        fprintf('\n========== 统计信息 ==========\n');
        fprintf('OptimalCost 最佳值   : %.4f\n', bestCost);
        fprintf('OptimalCost 中位数   : %.4f\n', medianCost);
        fprintf('OptimalCost 极差     : %.4f\n', rangeCost);
        fprintf('OptimalCost 平均值   : %.4f\n', avgCost);
        fprintf('OptimalCost 标准差   : %.4f\n', stdCost);
