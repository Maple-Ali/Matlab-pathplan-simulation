...
自主导航要求移动机器人在障碍物环境中以最优顺序访问一系列目标点，这一多目标点遍历路径规划问题在无人公交接驳、物流配送和基础设施巡检等场景中至关重要。该问题同时耦合两个子问题：一是在杂乱环境中寻找点对之间的无碰撞路径，二是确定目标点的最优访问顺序。现有研究往往只处理其中一个子问题——旅行商求解器默认欧氏距离，路径规划器则只优化单对点之间的路由。针对这一空白，本文提出一种将两者统一的协同优化框架。首先，基于跳点搜索（Jump Point Search, JPS）的改进A*全局规划器将障碍信息编码为旅行商代价矩阵，JPS利用栅格对称性跳过冗余节点扩展；随后结合安全感知的路径简化（拐点提取、贪心扫描与中间点探索）以及弧长参数化三次样条平滑。所得开放旅行商问题（起点与终点固定且不重合）由一种改进蚁群优化算法求解：通过虚拟节点法将其转化为闭环旅行商问题，并融合最大-最小蚁群系统的信息素管理、蚁群系统的伪随机转移规则、候选列表加速的可变邻域下降局部搜索，以及S形渐进调度和自适应停止准则。在三个测试环境上的实验表明，基于JPS的规划器相较Dijkstra最多减少83\%的节点扩展、搜索速度提升约17倍，并生成更短的路径；参数敏感性分析确定了一组均衡默认配置；消融实验表明可变邻域下降局部搜索与最大-最小蚁群系统信息素边界是保证求解质量的两个关键组件；所提出的求解器在kroB100基准上以97.5\%的命中率达到最优代价，显著优于标准蚁群优化、遗传算法和模拟退火，并表现出更高的稳定性。
...
综上，A*类方法可给出高质量的点对点路径，开放式旅行商元启发式则擅长组合排序，二者单独使用均难以覆盖障碍环境下的完整多目标点遍历任务。将基于跳点搜索的全局规划、安全感知后处理与开放旅行商蚁群优化相集成，可在保留几何可行性的同时提升访问顺序优化的质量。据我们所知，尚无先前工作同时满足以下三点：（1）基于跳点搜索的改进栅格全局规划器与安全感知路径后处理；（2）融合候选列表加速VND和S形渐进局部搜索的开放旅行商蚁群优化求解器；（3）利用简化路径代价实现精确组合优化的协同代价矩阵接口。因此，本文提出了相应的协同优化框架。
...
为克服单一路径规划器无法优化访问顺序、以及无约束旅行商模型忽略障碍代价的局限，本节提出一种由两个阶段组成的启发式协同框架：第一阶段采用改进型A*全局路径规划器（AStar\_v3\_1）在栅格地图上搜索点对点无碰撞最短路径，并通过安全距离感知的路径简化与三次样条平滑生成高质量连续路径；第二阶段以该阶段输出的代价矩阵为输入，由改进型蚁群优化求解器（TSP\_ACO\_v2\_4）求解多目标点的最优访问序列。两阶段通过代价矩阵实现耦合，共同完成从几何寻径到组合排序的完整流程。第~\ref{subsec:jps} 节和第~\ref{subsec:post} 节介绍全局路径规划及后处理，第~\ref{subsec:aco} 节详述开放旅行商求解算法，第~\ref{subsec:coop} 节阐述两层协同框架的整体设计。
...
\centering
\includegraphics[width=3.0in]{figures/fig01_jps_pruning}
...
在阐明所提框架的原理与机制之后，本节通过一组仿真验证其有效性。为从不同维度评估算法性能，设置四组实验：（1）改进的A*全局规划器（基于JPS）与基线规划器的性能对比；（2）改进蚁群优化求解器的参数敏感性分析；（3）隔离蚁群优化各组件贡献的消融实验；（4）不同旅行商求解器与改进A*规划器协同完成多目标点遍历的对比实验。所有对比均在统一参数策略与多次独立运行下进行，以兼顾结果质量与统计可靠性。
...
\toprule
\multirow{2}{*}{参数} & \multicolumn{5}{c}{参数水平} \\
\cmidrule(lr){2-6}
 & 水平1 & 水平2 & 水平3 & 水平4 & 水平5 \\
\midrule
...
\toprule
取值等级 & 地图 & 最优代价 & 平均收敛代价 & 成功率(\%) & 平均时间(ms) & 标准差 & 取值等级 & 地图 & 最优代价 & 平均收敛代价 & 成功率(\%) & 平均时间(ms) & 标准差 \\
\hline
\multirow{3}{*}{1/3/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 9.1 & 0 & \multirow{3}{*}{3/3/4/4/2} & Map~1 & 351.44 & 351.44 & 100 & 24.9 & 0 \\
& Map~2 & 620.15 & 620.34 & 80 & 148.0 & 0.44 & & Map~2 & 620.15 & 620.15 & 100 & 309.0 & 0 \\
& kroB100 & 22141.00 & 22145.03 & 92.5 & 250.5 & 14.66 & & kroB100 & 22141.00 & 22142.45 & 97.5 & 350.1 & 9.17 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{2/3/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 15.1 & 0 & \multirow{3}{*}{3/3/5/4/2} & Map~1 & 351.44 & 351.44 & 100 & 28.1 & 0 \\
& Map~2 & 620.15 & 620.27 & 85 & 204.7 & 0.29 & & Map~2 & 620.15 & 620.15 & 100 & 356.3 & 0 \\
& kroB100 & 22141.00 & 22143.90 & 95 & 254.8 & 12.80 & & kroB100 & 22141.00 & 22141.00 & 100 & 407.1 & 0 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{3/3/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 27.2 & 0 & \multirow{3}{*}{3/3/3/1/2} & Map~1 & 351.44 & 351.44 & 100 & 32.1 & 0 \\
& Map~2 & 620.15 & 620.15 & 100 & 241.5 & 0 & & Map~2 & 620.15 & 620.15 & 100 & 355.5 & 0 \\
& kroB100 & 22141.00 & 22142.45 & 97.5 & 347.9 & 9.17 & & kroB100 & 22141.00 & 22141.00 & 100 & 474.8 & 0 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{4/3/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 40.4 & 0 & \multirow{3}{*}{3/3/3/2/2} & Map~1 & 351.44 & 351.44 & 100 & 40.0 & 0 \\
& Map~2 & 620.15 & 620.15 & 100 & 371.1 & 0 & & Map~2 & 620.15 & 620.17 & 97.5 & 344.4 & 0.13 \\
& kroB100 & 22141.00 & 22141.00 & 100 & 480.5 & 0 & & kroB100 & 22141.00 & 22141.00 & 100 & 421.5 & 0 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{5/3/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 52.7 & 0 & \multirow{3}{*}{3/3/3/3/2} & Map~1 & 351.44 & 351.44 & 100 & 27.5 & 0 \\
& Map~2 & 620.15 & 620.15 & 100 & 336.3 & 0 & & Map~2 & 620.15 & 620.15 & 100 & 272.8 & 0 \\
& kroB100 & 22141.00 & 22142.45 & 97.5 & 628.7 & 9.17 & & kroB100 & 22141.00 & 22142.45 & 97.5 & 357.8 & 9.17 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{3/1/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 37.8 & 0 & \multirow{3}{*}{3/3/3/5/2} & Map~1 & 351.44 & 351.44 & 100 & 21.7 & 0.02 \\
& Map~2 & 620.15 & 620.17 & 97.5 & 426.5 & 0.13 & & Map~2 & 620.15 & 620.21 & 92.5 & 211.6 & 0.22 \\
& kroB100 & 22141.00 & 22141.00 & 100 & 548.4 & 0 & & kroB100 & 22141.00 & 22145.35 & 92.5 & 342.0 & 15.47 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{3/2/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 27.2 & 0 & \multirow{3}{*}{3/3/3/4/1} & Map~1 & 351.44 & 351.44 & 100 & 37.8 & 0 \\
& Map~2 & 620.15 & 620.15 & 100 & 311.8 & 0 & & Map~2 & 620.15 & 620.17 & 97.5 & 217.8 & 0.13 \\
& kroB100 & 22141.00 & 22141.00 & 100 & 475.7 & 0 & & kroB100 & 22141.00 & 22193.30 & 30 & 786.8 & 37.45 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{3/4/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 30.5 & 0 & \multirow{3}{*}{3/3/3/4/3} & Map~1 & 351.44 & 351.44 & 100 & 33.8 & 0 \\
& Map~2 & 620.15 & 620.19 & 95 & 257.0 & 0.18 & & Map~2 & 620.15 & 620.15 & 100 & 243.2 & 0 \\
& kroB100 & 22141.00 & 22143.90 & 95 & 296.7 & 12.80 & & kroB100 & 22141.00 & 22143.90 & 95 & 404.3 & 12.80 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{3/5/3/4/2} & Map~1 & 351.44 & 351.44 & 100 & 38.5 & 0 & \multirow{3}{*}{3/3/3/4/4} & Map~1 & 351.44 & 351.44 & 100 & 33.5 & 0 \\
& Map~2 & 620.15 & 620.75 & 62.5 & 232.1 & 1.17 & & Map~2 & 620.15 & 620.17 & 97.5 & 286.0 & 0.13 \\
& kroB100 & 22141.00 & 22180.55 & 50 & 351.7 & 42.06 & & kroB100 & 22141.00 & 22142.45 & 97.5 & 390.8 & 9.17 \\ \cline{2-7}\cline{9-14}
\multirow{3}{*}{3/3/1/4/2} & Map~1 & 351.44 & 351.44 & 100 & 44.4 & 0 & \multirow{3}{*}{3/3/3/4/5} & Map~1 & 351.44 & 351.44 & 100 & 28.3 & 0 \\
& Map~2 & 620.15 & 620.71 & 65 & 217.1 & 1.05 & & Map~2 & 620.15 & 620.19 & 95 & 287.7 & 0.18 \\
& kroB100 & 22141.00 & 22157.05 & 80 & 365.4 & 35.39 & & kroB100 & 22141.00 & 22142.45 & 97.5 & 487.7 & 9.17 \\ \cline{2-7}
\multirow{3}{*}{3/3/2/4/2} & Map~1 & 351.44 & 351.44 & 100 & 32.2 & 0 & & & & & & & \\
& Map~2 & 620.15 & 620.35 & 75 & 247.2 & 0.35 & & & & & & & \\
& kroB100 & 22141.00 & 22143.40 & 95 & 294.5 & 10.83 & & & & & & & \\
\toprule
\end{tabular}
}
\end{table*}

\textbf{分析。} 结果揭示了若干重要的敏感性特征。首先，蚂蚁数量$n_{\mathrm{ants}}$具有中等影响：在$n_{\mathrm{ants}} = 40$（默认值）时，算法在Map~2上达到100\%成功率、kroB100上达97.5\%，而减至10只蚂蚁使kroB100成功率降至92.5\%并增加平均代价。其次，贪心概率$q_0$在默认值0.45处呈现明确最优：增至0.9（近纯贪心）导致严重退化，kroB100成功率降至50\%、标准差增至42.06，表明过早收敛。第三，后期局部搜索比例$y_{\max}$至关重要：减至0.10使Map~2成功率降至65\%、kroB100降至80\%，证实密集后期局部搜索对解质量不可或缺。第四，精英比率$r_{\mathrm{elite}}$和候选列表大小$k$的敏感性相对温和，候选列表大小$k$是最不敏感的参数——但过小的$k = 3$使kroB100成功率降至30\%，表明最小候选列表对有效VND剪枝是必要的。这表明默认配置在收敛质量、稳定性与运行时间之间取得了良好折中，故后续实验均采用该组参数。各参数水平的代价与时间对比如图~\ref{fig:sens_cost}和图~\ref{fig:sens_time}所示，默认参数下规划路径如图~\ref{fig:sens_paths}所示。
...
\centering
\includegraphics[width=1.55in]{figures/fig12_default_m1}\hfil
\includegraphics[width=1.55in]{figures/fig12_default_m2}\\
\includegraphics[width=2.5in]{figures/fig12_default_kro}
...
\textbf{结果。} 表~\ref{tab:ablation}报告了所有地图上的完整消融结果。消融过程可视化见图~\ref{fig:abl_conv}--\ref{fig:abl_cost_time}。
...
\textbf{分析。} 消融结果揭示了组件重要性的清晰层次。最关键的组件是VND局部搜索（组3）：禁用后kroB100成功率崩溃至0\%，平均代价从22142.45增至22566.98（退化1.9\%），标准差膨胀至239.41。这证实VND对逃脱局部最优和实现近优解不可或缺。第二关键的组件是MMAS信息素边界（组6）：移除后Map~1成功率降至30\%、Map~2降至10\%，代价方差显著，证明信息素边界控制对防止过早收敛至关重要。伪随机转移规则（组2）和S曲线调度（组4）的影响更温和：移除伪随机规则实际上维持了100\%成功率（以稍长运行时间为代价），而移除S曲线使kroB100性能轻微退化（成功率从97.5\%降至90\%）。候选列表加速（组5）对解质量的影响最小——这与其作为纯计算加速器的角色一致——同时提供运行时间收益。综上，所提算法的性能可归因于以下关键因素：其一，VND局部搜索系统性地精化回路，是逼近最优代价的主要来源；其二，MMAS信息素边界抑制过早收敛，维持了种群多样性；其三，伪随机转移、S形调度与候选列表剪枝则主要提升收敛速度并降低计算开销。消融收敛曲线、代价分布与平均代价/时间对比分别如图~\ref{fig:abl_conv}--\ref{fig:abl_cost_time}所示。
...
\begin{figure*}[!t]
\centering
\subfloat[Map~1]{\includegraphics[width=0.3\textwidth]{figures/fig15_abl_costtime_map1}}\hfil
\subfloat[Map~2]{\includegraphics[width=0.3\textwidth]{figures/fig15_abl_costtime_map2}}\hfil
\subfloat[kroB100]{\includegraphics[width=0.3\textwidth]{figures/fig15_abl_costtime_kro}}
...
局部搜索比例$y_{\max}=0.4$；\newline
局部搜索过渡区间$=[0,30]$；\newline
局部搜索精英蚂蚁占比$r_{\mathrm{elite}}=0.7$；\newline
候选邻居数$k=9$ \\
\addlinespace
...
\begin{figure*}[!t]
\centering
\subfloat[Map~1]{\includegraphics[width=0.3\textwidth]{figures/fig19_tsp_costtime_map1}}\hfil
\subfloat[Map~2]{\includegraphics[width=0.3\textwidth]{figures/fig19_tsp_costtime_map2}}\hfil
\subfloat[kroB100]{\includegraphics[width=0.3\textwidth]{figures/fig19_tsp_costtime_kro}}
...
在路径规划阶段，跳点搜索（JPS）利用均匀代价栅格中的对称性剪枝冗余节点，仅在路径方向改变或出现强制选择处扩展，从而在保持最优性的同时大幅缩小搜索树。安全感知的路径简化通过拐点提取、贪心正向扫描、中间点探索与路径对比生成最小安全航点子集；弧长参数化三次样条平滑进一步将简化折线转化为适合物理机器人执行的连续轨迹。

在序列优化阶段，改进蚁群优化求解器集成了六个机制。虚拟节点法将开放旅行商问题转化为标准闭环旅行商问题，使起点与终点完整参与信息素更新与局部搜索；具有动态边界的最大-最小蚁群系统信息素管理抑制过早收敛并维持探索多样性；蚁群系统风格的伪随机转移规则在受控贪心与随机探索之间取得平衡；候选列表加速的可变邻域下降将局部搜索复杂度由$O(K^2)$降至$O(K \cdot k)$，使超过100个目标点实例上的密集邻域搜索成为可能；S形渐进调度在迭代过程中逐步由探索转向利用；基于种群同质性与最优解停滞的自适应停止准则则避免无效计算。

实验从路径长度、计算时间、扩展节点数、收敛代价、成功率与标准差等指标验证了上述设计。基于JPS的规划器相较Dijkstra和标准A*最多减少83\%的节点扩展（Map~3上48个对比566个），在Map~2上将规划时间由202.29~ms降至11.85~ms，并取得更短路径。参数敏感性分析表明默认超参数处于均衡区，其中贪心概率$q_0$与后期局部搜索比例$y_{\max}$最为敏感。消融实验呈现出清晰的组件层次：VND局部搜索与MMAS信息素边界共同支撑解质量，禁用任一组件会使成功率由近100\%降至30\%以下；伪随机转移、S形调度与候选列表则主要改善收敛速度与计算效率。在多目标点遍历对比中，所提MMAS-VND-CL求解器在kroB100上以97.5\%的命中率达到最优代价（平均代价22142.45），较最优基线标准蚁群优化（平均代价24435.83）改善约10.4\%，且标准差比遗传算法小一个数量级以上，体现出良好的稳定性。

本文工作仍有若干局限，亦指明了后续研究方向。第一，当前框架假设障碍地图静态且完全已知，面向任务执行中动态出现或移动的障碍物尚需在线重规划机制；第二，地图表示限于二维栅格，扩展至含高程约束的三维地形有望拓宽其在空中与水下平台上的适用性；第三，候选列表规模$k$等超参数目前固定不变，自适应或基于学习的参数控制有望进一步提升跨实例泛化能力；第四，框架目前面向单机器人任务，若扩展到多机器人协同多目标点遍历（共享目标集并避免机器人间碰撞），将具有更强的实用价值；第五，完整$O(N^2)$代价矩阵在目标规模很大时计算开销较高，可借助距离预言机近似或增量代价精化加以缓解。
