...
%TSP_ACO_V2_2 蚁群 + VND 局部搜索 + 停滞触发 Double-bridge 求解 TSP
%   基于 TSP_ACO_v2_1，核心改进:
%     M. Double-bridge 突变改为停滞触发: 连续 dbStagThr 代无改进时，
%        对局部搜索比例内的蚂蚁施加 DB + VND，而非对底部蚂蚁每代执行
%     B. 自适应停止改为 DB 扰动失败计数: 连续 dbFailLimit 次 DB+VND
%        扰动未改进全局最优则停止
...
% ===== 方案 M：Double-bridge 突变 (停滞触发) =====
dbStagThr = 50;           % 连续停滞此代数时触发 DB+VND 扰动
dbFailLimit = 1;          % 连续 DB 扰动失败次数上限
dbEliteRatio = 0.05;      % DB 突变精英比例 (固定前此比例的全部蚂蚁 + 随机探索蚂蚁)
...
for iter = 1:nIter
    ...
    % ---- 方案 M：停滞时 DB 突变 (仅对 dbElite + 随机探索蚂蚁) ----
    if triggerDB
        nDBelite = round(nAnts * dbEliteRatio);           % 固定前 dbEliteRatio 的全部蚂蚁
        dbAnts = [sortIdx(1:nDBelite); sortIdx(randPool)]; % 顶部精英 + 随机探索蚂蚁
        dbAnts = unique(dbAnts, 'stable');                 % 去重 (随机蚂蚁可能已在前列)

        for idx = 1:length(dbAnts)
            a = dbAnts(idx);
            antTours{a} = doubleBridgeLocal(antTours{a});
            [antTours{a}, antCosts(a)] = vndSearchLocal(antTours{a}, midIdx, costMatrix, nPts);
        end
    end
    ...
    if enableAdaptiveStop && iter >= minIter
        ...
        if triggerDB && ~improved
            ...
            if dbFailCount >= dbFailLimit
                stopReason = sprintf('DB扰动连续失败(%d次)', dbFailCount);
                break;
            end
        end
    end
...
% =========================================================================
function tour = doubleBridgeLocal(tour)
nMid = length(tour);
