function [bestOrder, bestCost, history] = TSP_SA_v0_X(costMatrix, nPts)
%TSP_SA_V0_X 经典模拟退火算法求解 TSP（虚拟节点法：起点终点固定 + 中间点全排列优化）
%   在原 TSP_SA_v0 基础上，将起点终点固定问题通过虚拟节点转化为闭合环路 TSP。
...
for t = 1:nMid
    ...
    visited(bestJ) = true;
    midOrder(t) = midIdx(bestJ);
    curNode = midIdx(bestJ);
end
