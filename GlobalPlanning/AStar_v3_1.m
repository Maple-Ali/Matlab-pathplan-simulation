function path = AStar_v3_1(map, startGrid, goalGrid, delay, callback)
%ASTAR_V3_1 A* 全局路径规划 — 改进版 v3_1（Jump Point Search，标准启发式）
%   path = AStar_v3_1(map, startGrid, goalGrid, delay, callback)
%   map: Map 对象
...
while heapSize > 0
    ...
    if cr == goalGrid(1) && cc == goalGrid(2)
        path = reconstructJPSPath(parent, startGrid, goalGrid, occGrid);
        if ~isempty(callback)
...
% JPS 无路径 → 回退到标准 A*（二叉堆版，处理窄迷宫等 JPS 失效场景）
path = AStar_v0(map, startGrid, goalGrid, 0, callback);
...
end  % AStar_v3_1 主函数结束

% ==================== 路径重建（JPS 插值） ====================

function path = reconstructJPSPath(parent, startGrid, goalGrid, occGrid)
...
%   JPS 的 parent 记录的是跳点之间的跳跃关系，需要沿直线插值补充中间格子
    ...
    % 对角线拐角穿越修正：如果对角线移动穿越了障碍物角，替换为 L 形路径
    fixedPath = path(1, :);
    for i = 1:size(path, 1) - 1
        r1 = path(i, 1); c1 = path(i, 2);
        r2 = path(i+1, 1); c2 = path(i+1, 2);
        dr = r2 - r1; dc = c2 - c1;
        if dr ~= 0 && dc ~= 0
            % 对角线移动：检查是否穿越障碍物角
            crossR = ~isBlockedStatic(occGrid, r2, c1);  % 先走行再走列
            crossC = ~isBlockedStatic(occGrid, r1, c2);  % 先走列再走行
            if ~crossR && ~crossC
                % 两侧都堵（不应出现），保留对角
                fixedPath = [fixedPath; r2, c2]; %#ok<AGROW>
            elseif crossR && ~crossC
                % (r2,c1) 可走：先走到 (r2,c1) 再到 (r2,c2)
                fixedPath = [fixedPath; r2, c1; r2, c2]; %#ok<AGROW>
            elseif ~crossR && crossC
                % (r1,c2) 可走：先走到 (r1,c2) 再到 (r2,c2)
                fixedPath = [fixedPath; r1, c2; r2, c2]; %#ok<AGROW>
            else
                % 两侧都可走，选择不穿越障碍角的路径
                % 优先走不贴着障碍物的一侧
                fixedPath = [fixedPath; r2, c2]; %#ok<AGROW>
    ...
    % 去除连续重复节点
    if size(path, 1) > 1
        keep = [true; any(diff(path, 1, 1) ~= 0, 2)];
        path = path(keep, :);
    end
end

function blocked = isBlockedStatic(occGrid, r, c)
    if r < 1 || r > size(occGrid, 1) || c < 1 || c > size(occGrid, 2)
        blocked = true;
    else
        blocked = occGrid(r, c);
    end
